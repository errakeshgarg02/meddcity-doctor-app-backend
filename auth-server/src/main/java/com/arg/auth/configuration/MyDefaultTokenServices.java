...
public class MyDefaultTokenServices extends DefaultTokenServices {
    ...
    public OAuth2AccessToken refreshAccessToken(String refreshTokenValue, TokenRequest tokenRequest)
        ...
        if (this.authenticationManager != null && !authentication.isClientOnly()) {
            ...
            boolean anyMatch = authentication.getAuthorities().stream()
                    .anyMatch(f -> f.getAuthority().equalsIgnoreCase(RoleEnum.ROLE_ADMIN.getValue()));

            if (anyMatch) {
                user = new PreAuthenticatedAuthenticationToken(
                        user.getPrincipal(), "",
                        Collections.singleton(AdminAsCustomerTokenGranter.ROLE_ADMIN_AS_CUSTOMER));
            }
