...
import com.arg.auth.exception.MfaRequiredException;
...
public class PasswordTokenGranter extends AbstractTokenGranter {
    ...
    protected OAuth2Authentication getOAuth2Authentication(ClientDetails client,
        ...
        if (userAuth != null && userAuth.isAuthenticated()) {
            ...
            AuthUser user = (AuthUser) userAuth.getPrincipal();
            if (user.isMfaEnabled()) {
                mfaService.sendVerificationCode(user, MessageType.MFA);
                userAuth = new UsernamePasswordAuthenticationToken(userAuth.getPrincipal(),
                        password, Collections.singleton(PRE_AUTH));
                OAuth2AccessToken accessToken = getTokenServices()
                        .createAccessToken(new OAuth2Authentication(
