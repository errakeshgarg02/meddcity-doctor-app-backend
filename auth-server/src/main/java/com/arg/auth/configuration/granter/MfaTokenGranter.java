...
public class MfaTokenGranter extends AbstractTokenGranter {
    ...
    protected OAuth2Authentication getOAuth2Authentication(ClientDetails client,
        ...
        if (mfaToken != null) {
            OAuth2Authentication authentication = this
                    .loadAuthentication(mfaToken);
            final String username = authentication.getName();
            if (parameters.containsKey("mfa_code")) {
                String code = parameters.get("mfa_code");
    ...
    private OAuth2Authentication getAuthentication(TokenRequest tokenRequest,
        ...
        user = new UsernamePasswordAuthenticationToken(user.getPrincipal(),
                authentication.getUserAuthentication().getCredentials(),
                user.getAuthorities());

        authentication = new OAuth2Authentication(
                authentication.getOAuth2Request(), user);
    ...
    private OAuth2Authentication refreshAuthentication(
        ...
        return new OAuth2Authentication(clientAuth,
